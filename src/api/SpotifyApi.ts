...
export default class SpotifyApi {
  ...
  public static getAuthorizeURL(): string {
    ...
    return (
      ...
      "&redirect_uri=" +
      encodeURIComponent(
        `${import.meta.env.VITE_SPOTIFY_REDIRECT_URI}/authenticate-spotify`
      )
    );
  }
  ...
  public getTotalArtistCount(limit: number = 1): Promise<number> {
    return new Promise((resolve, reject) => {
      this.spotify.getFollowedArtists(
        { limit },
  ...
  public async getArtistsAlbums(artistId: string): Promise<any> {
    const moment = (await import("moment")).default;

    return new Promise((resolve, reject) => {
      this.spotify.getArtistAlbums(
        artistId,
        (error: any, artistsAlbums: any) => {
          SpotifyApi.processError(error, reject);
  ...
  public getRelatedArtists(artistId: string): Promise<any> {
    return new Promise((resolve, reject) => {
      this.spotify.getArtistRelatedArtists(
        artistId,
        (error: any, relatedArtists: any) => {
          SpotifyApi.processError(error, reject);
  ...
  public getArtistsInfo(
    artistIds: string[]
  ): Promise<SpotifyApi.ArtistObjectFull[]> {
    return new Promise((resolve, reject) => {
      this.spotify
        .getArtists(artistIds)
        .then((artistsInfo: SpotifyApi.MultipleArtistsResponse) => {
  ...
  public getTrackInfo(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        const trackInfo: SpotifyApi.SingleTrackResponse =
          await this.spotify.getTrack(trackId);
        resolve(trackInfo);
      } catch (error) {
        SpotifyApi.processError(error, reject);
      }
  ...
  public getTopTracksByCountry(
    artistId: string,
    countryCode: string
  ): Promise<any> {
    return new Promise((resolve, reject) => {
      this.spotify.getArtistTopTracks(
        artistId,
        countryCode,
        (error: any, response: any) => {
          SpotifyApi.processError(error, reject);
  ...
  public getIsCurrentUserFollowingArtists(artistIds: string[]) {
    return new Promise((resolve, reject) => {
      this.spotify.isFollowingArtists(
        artistIds,
        (error: any, response: any) => {
          SpotifyApi.processError(error, reject);
          resolve(response);
  ...
  public async getCurrentUserAllFollowedArtists(): Promise<any> {
    return new Promise(async (resolve, reject) => {
      ...
      const numLoops =
        totalArtistCount > limit ? Math.ceil(totalArtistCount / limit) : 1;
      let after = "";
      let options = { limit };
      ...
      for (let i = 0; i < numLoops; i++) {
        ...
        try {
          ...
          followedArtists = [...followedArtists, ...response.artists.items];
  ...
  public getCurrentUserSavedAlbums(
    limit: number = 10,
    offset: number = 0
  ): Promise<any> {
    return new Promise((resolve, reject) => {
      this.spotify.getMySavedAlbums(
        { limit, offset },
        (error: any, response: any) => {
          SpotifyApi.processError(error, reject);
  ...
  public getRecentlyPlayedTracks(
    ...
    const getRecentlyPlayedTracks = (
      ...
      this.spotify.getMyRecentlyPlayedTracks(
        options,
        (error: any, recentlyPlayedTracks: any) => {
          SpotifyApi.processError(error, reject);
  ...
  public getAvailableGenres(): Promise<SpotifyApi.AvailableGenreSeedsResponse> {
    return new Promise((resolve, reject) => {
      this.spotify.getAvailableGenreSeeds(
        (error: any, response: SpotifyApi.AvailableGenreSeedsResponse) => {
          SpotifyApi.processError(error, reject);
          resolve(response);
        }
      );
  ...
  public getTrackFeatures(trackId: string): Promise<any> {
    return new Promise(async (resolve, reject) => {
      try {
        const audioFeatures: SpotifyApi.AudioFeaturesResponse =
          await this.spotify.getAudioFeaturesForTrack(trackId);
  ...
  public getTracksFeatures(
    trackIds: string[]
  ): Promise<SpotifyApi.AudioFeaturesObject[]> {
    return new Promise((resolve, reject) => {
      this.spotify
        .getAudioFeaturesForTracks(trackIds)
        .then((response: SpotifyApi.MultipleAudioFeaturesResponse) => {
