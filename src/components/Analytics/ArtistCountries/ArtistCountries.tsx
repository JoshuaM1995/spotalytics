...
const ArtistCountries = () => {
  const [artistCountriesData, setArtistCountriesData] = useState<
    ArtistCountry[]
  >([]);
  ...
  useAsyncEffect(async () => {
    const artistCountriesDataCache: ArtistCountry[] = cache.get(
      CacheKey.ARTIST_COUNTRIES_DATA
    );
    const unknownArtistsDataCache: UnknownArtist[] = cache.get(
      CacheKey.UNKNOWN_ARTISTS
    );
    ...
    if (!artistCountriesDataCache) {
      ...
      const allFollowedArtists: any[] =
        await spotifyApi.getCurrentUserAllFollowedArtists();
      allFollowedArtists.forEach((artist) => {
        promises.push(
          axios.get(`${AUDIODB_BASE_URL}/search.php?s=${artist.name}`, {
            headers: null,
          })
        );
      });
      ...
      axios.all(promises).then(
        axios.spread((...responses: any) => {
            ...
            if (response.data.artists !== null) {
              if (response.data.artists[0].strCountryCode) {
                artistCountries.push({
                  country:
                    alpha2to3[
                      response.data.artists[0].strCountryCode.toLowerCase()
                    ] ?? "N/A",
                });
              } else {
                unknownArtists.push({
                  artist: response.data.artists[0].strArtist,
                });
              }
            }
          });
          ...
          const artistWithMostCountries = _.sortBy(
            artistCountriesNew,
            "value"
          ).reverse()[0];
  ...
  return (
    <Container
      ...
      style={{ height: window.outerHeight - 150 }}
    >
      <div style={{ textAlign: "center", margin: "15px 0 30px 0" }}>
        <h3>Artist's Countries</h3>
        <h5>
          Displays which countries have the most artists based on the artists
          you follow.
        </h5>
      </div>
        ...
        tooltip={({ feature }: any) => {
          ...
          return (
            <div className="nivo-tooltip">
              {data && (
                <>
                  {label}: {data.value} {data.value > 1 ? "Artists" : "Artist"}
                </>
              )}
