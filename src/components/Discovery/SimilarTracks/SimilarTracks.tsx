...
const SimilarTracks = () => {
  ...
  useAsyncEffect(async () => {
    ...
    if (trackId && !seedTrackName) {
      ...
      setTracks([{ value: trackId, label: trackInfo.name }]);
  ...
  const selectTrack = async (trackId: string) => {
    ...
    let recommendationOptions = _.omit(response, [
      "analysis_url",
      "id",
      "track_href",
      "type",
    ...
    recommendationOptions = _.mapKeys(
      recommendationOptions,
      (value, key) => `target_${key}`
    );
    recommendationOptions = {
      ...recommendationOptions,
      ...{ seed_tracks: trackId, limit: 50 },
    };
    const tracksResponse = await spotifyApi.getFilteredRecommendations(
      recommendationOptions
    );
    let recommendedTracks: RecommendedTrack[] = tracksResponse.tracks.map(
      (track: any) => {
        return {
          id: track.id,
          trackUri: track.uri,
  ...
  return (
    <Page title="Similar Tracks">
      <h5>
        Search for and select a track, and we'll analyze its audio properties
        and create a playlist with similar tracks.
      </h5>
      <br />
